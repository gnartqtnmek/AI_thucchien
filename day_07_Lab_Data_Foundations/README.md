...
## Benchmark Tự Động Precision@k

Để so sánh 3 chiến lược chunking (`fixed_size`, `sentence`, `recursive`) một cách tự động trên 5 câu hỏi chuẩn:

```bash
python benchmark_precision.py --top-k 3 --provider mock
```

Nếu bạn đã chuẩn bị bộ query riêng (ví dụ `question.txt`):

```bash
python benchmark_precision.py --cases-file question.txt --top-k 3 --provider mock
```

Nếu muốn xuất kết quả dạng markdown để chèn vào báo cáo:

```bash
python benchmark_precision.py --top-k 3 --provider mock --output report/precision_benchmark.md
```

Ghi chú:
- `--provider auto` (mặc định) sẽ đọc từ biến môi trường `EMBEDDING_PROVIDER`.
- `--provider mock` chạy nhanh và ổn định cho mục đích so sánh strategy.
- `--cases-file` cho phép benchmark theo file query của nhóm thay vì danh sách hardcode.
- Script sẽ tự in `Best strategy` dựa trên `Avg Precision@k` và `Hit Rate`.
