...
def real_llm(prompt: str) -> str:
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "Bạn là trợ lý ảo hỗ trợ trả lời câu hỏi dựa trên tài liệu được cung cấp."},
...
def run_manual_demo(question: str | None = None, sample_files: list[str] | None = None) -> int:
    ...
    print("=== Chạy thử nghiệm với RECURSIVE CHUNKER ===")
    ...
    if provider == "local":
        try: embedder = LocalEmbedder(model_name=os.getenv("LOCAL_EMBEDDING_MODEL", LOCAL_EMBEDDING_MODEL))
        except Exception: embedder = _mock_embed
    elif provider == "openai":
        try: embedder = OpenAIEmbedder(model_name=os.getenv("OPENAI_EMBEDDING_MODEL", OPENAI_EMBEDDING_MODEL))
        except Exception: embedder = _mock_embed
    else:
        embedder = _mock_embed

    # KHỞI TẠO CHIẾN LƯỢC: RECURSIVE CHUNKER
    ...
    for doc in docs:
        ...
        for i, text_chunk in enumerate(chunks):
            chunked_docs.append(Document(id=f"{doc.id}_chunk_{i}", content=text_chunk, metadata=doc.metadata))

    store = EmbeddingStore(collection_name="manual_test_store", embedding_fn=embedder)
    store.add_documents(chunked_docs)

    print(f"\nStored {store.get_collection_size()} chunks in EmbeddingStore")
    print(f"\n=== Kết quả tìm kiếm (Top {SEARCH_TOP_K}) ===")
    search_results = store.search(query, top_k=SEARCH_TOP_K)
    for index, result in enumerate(search_results, start=1):
        print(f"{index}. score={result['score']:.3f} source={result['metadata'].get('source')}")
        print(f"   preview: {result['content'][:100].replace(chr(10), ' ')}...")
